...
* Para criar array de 2 dimensões, siga essa exemplo : 
	...
	```
* Para criar uma array de 2 dimensões vazia com um tamanho definido é desse jeito : 
	```CS
...
### Criando List : 
* List diferente de Arrays, podem ter seu tamanho modificado após sua atribuição, não é recomendado criar listas de 2 dimensões. 
...
* 
	...
	```
* Para [clonar o conteúdo](link_Memoria_das_coisas) duma lista, siga o exemplo :
	```CS
	List<int> tes = new List<int>(){1,2,3}; // lista com conteudo
	List<int> tes2 = new List<int>(tes); // clonando conteudo de "tes"
	``` 
...
* "lista_name.Count" , retorna tantos itens a **lista** possui. 

### Criando Enum :
* Enum é uma maneira de armazenar dados simples, onde sua "key" é seu "valor".
* EX :
	```CS
	Enum Raridade
	{
	...
	Raridade pedra_raridade = Raridade.comum;
	```
...
## [-- Memoria das Coisas --](link_Memoria_das_coisas)

## [-- Generics Function/Class --](link_Generics)
## Conceitos Básicos :
### Criando arquivo C++++ :
